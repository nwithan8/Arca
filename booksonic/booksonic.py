...
class Booksonic(commands.Cog):
    ...
    def request(self, cmd, params):
        salt, token = self.makeToken()
        response = requests.get(BOOKSONIC_URL + "/booksonic/rest/" + cmd + ".view?u=" + BOOKSONIC_USER + "&t=" + token + "&s=" + salt + "&v=1.14&c=booksonic" + (("&" + str(params)) if params != None else ""))
    ...
    async def booksonic_add(self, ctx: commands.Context, user: discord.Member, booksonicUsername: str):
        ...
        """
        p = self.password(10)
        s = self.request('createUser','username=' + booksonicUsername + "&password=" + (DEFAULT_PASSWORD if USE_DEFAULT_PASSWORD else p) + "&email=" + DEFAULT_EMAIL + "&jukeboxRole=true&downloadRole=true&commentRole=true")
        if s == 'ok':
            ...
            await user.dm_channel.send("You have been added to " + str(BOOKSONIC_SERVER_NAME) + "!\n" +
                                       ...
                                       "Password: " + ((str(DEFAULT_PASSWORD) + "\nPlease change this password when you first log in.") if USE_DEFAULT_PASSWORD else str(p)) + ".\n" + 
                                       "Enjoy!")
