...
def add_embed_field(embed, title, outlet_name, link):
    embed.add_field(name=unescape(title), value=f"[*{outlet_name}*]({link})", inline=False)


def get_single_headline(category, outlet):
    f = feedparser.parse(feeds[category][outlet])
    return f['entries'][0]['title'], f['entries'][0].link


def get_headlines(category, outlet, number: int = 5):
    f = feedparser.parse(feeds[category][outlet])
    data = []
...
def get_outlet(data):
    if data.lower() in ['bbc', 'british broadcasting channel''bbc.com', 'cbbc']:
        ...
        return "NPR"
    elif data.lower() in ['nyt', 'new york times', 'ny times', 'nytimes', 'times', 'the times', 'nyt.com',
        ...
        return 'WSJ'
    elif data.lower() in ['wp', 'wapo', 'washington post', 'post', 'the post', 'washingtonpost',
...
def make_category_response_embed(embed_title: str, category: str, count: int = 5):
    embed = discord.Embed(title=embed_title)
    for o in feeds[category]:
        t, l = get_single_headline(category, o)
        add_embed_field(embed=embed, title=t, link=l, outlet_name=outlets[o])
    return embed
...
class News(commands.Cog):
    ...
    async def news_from(self, ctx: commands.Context, *, outlet: str):
        ...
        if not o:
            ...
            h = get_headlines("top", o, 5)
            for i in h:
                embed.add_field(name=unescape(i['title']),
    ...
    async def news_brief(self, ctx: commands.Context):
        ...
        Top news headlines
        """
        embed = make_category_response_embed(embed_title="News Brief", category='brief', count=5)
        await ctx.send(embed=embed)
    ...
    async def news_world(self, ctx: commands.Context):
        ...
        """
        embed = make_category_response_embed(embed_title="World News", category='world')
        await ctx.send(embed=embed)
    ...
    @news.command(name="u.s", aliases=["us", "america", "united states", "national"], pass_context=True,
                  case_insensitive=True)
    async def news_us(self, ctx: commands.Context):
        """
        U.S. news headlines
        """
        embed = make_category_response_embed(embed_title="U.S. News", category='us')
        await ctx.send(embed=embed)
    ...
    async def news_sports(self, ctx: commands.Context):
        ...
        """
        embed = make_category_response_embed(embed_title="Sports News", category='sports')
        await ctx.send(embed=embed)
