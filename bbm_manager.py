...
#Database Schema:
#trial_users
#(DiscordID BIGINT, PlexUsername 'VARCHAR(100)', ExpirationStamp INT, PlexEmail 'VARCHAR(320)')
#regular_users
#(DiscordID BIGINT, PlexUsername 'VARCHAR(100)', PlexEmail 'VARCHAR(320)', JoinTimestamp INT, EndOfMonth INT, SubType 'VARCHAR(5)')
...
#Discord settings
SERVER_ID = 472537215457689601
ADMIN_USERNAME = 'nwithan8'
ADMIN_ID = 233771307555094528
FRIENDLY_LOGGING = True
FRIENDLY_LOG_CHANNEL_ID = 605429370244300827
VERBOSE_LOGGING = True
VERBOSE_LOG_CHANNEL_ID = 595287458556411917
...
#subRoles = ["Winner", "Bot"]
exemptsubs = [233771307555094528 #Nate
] #Discord IDs for users who are exempt from subscriber checks/deletion
...
class BBM_Manager(commands.Cog):
    ...
    async def find_in_db(self, data, PlexOrDiscord):
        myConnection = mysql.connector.connect(host=hostname,user=username,passwd=password,db=database)
        result = None
        if myConnection.is_connected():
            cursor = myConnection.cursor(buffered=True)
    ...
    async def purge_winners(self, ctx):
        with open("monitorwinners.txt","r") as f:
    ...
    async def add_process(self, ctx, member, plexname):
        await self.log("Adding " + str(plexname), "v")
        await ctx.send("Adding " + plexname + ". Please wait about 60 seconds...")
        try:
            winner_role = discord.utils.get(self.bot.get_guild(SERVER_ID).roles, name="Winner")
            if winner_role in member.roles:
                f=open('monitorwinners.txt',"a")
                f.write(str(plexname)+",")
                f.close()
            await self.add_to_plex(plexname)
            await self.add_to_regular_db(member.id, plexname)
            await self.log("Discord user " + member.name + " (ID: " + str(member.id) + ") is Plex user " + plexname, "v")
            role = discord.utils.get(self.bot.get_guild(SERVER_ID).roles, name=afterApprovedRoleName)
            ...
            await ctx.send(member.mention + " You've been invited, " + plexname + ". Welcome to " + DISCORD_SERVER_NAME + "!")
        except plexapi.exceptions.BadRequest:
            await ctx.send(m.mention + ", " + plexname + " is not a valid Plex username.")
    ...
    async def on_ready(self):
        ...
        await self.log("BBM Manager ready.", "f")
        
    @commands.group(name="bbm",aliases=["BBM"],hidden=True,pass_context=True)
    ...
    @bbm.command(name="add")
    @commands.has_role("Admin")
    async def bbm_add(self, ctx: commands.Context, member: discord.member, plexUsername: str):
        await self.add_process(ctx, member, plexUsername)
        
    @bbm.command(name="remove")
    @commands.has_role("Admin")
    async def bbm_remove(self, ctx: commands.Context, plexUsername: str):
        await self.delete_from_plex(plexUsername)
        discord_id = self.find_in_db(plexUsername, "Discord")
        await ctx.send("<@"+ discord_id + "> (" + plexUsername + "), you have been removed from " + DISCORD_SERVER_NAME + ". To appeal this removal, please send a Direct Message to <@" + ADMIN_ID + ">")
        
    @commands.Cog.listener()
    async def on_reaction_add(self, reaction, user):
        if reaction.emoji.name == approvedEmojiName and user.name in ADMIN_USERNAME: #Add user to Plex and Tautulli
            await self.add_process(reaction.message.channel, reaction.message.author, reaction.message.content.strip())
