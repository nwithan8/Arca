...
import requests
import json
import media_server.deluge.settings as settings

WEBUI_PASSWORD = settings.WEBUI_PASSWORD
WEB_URL = settings.WEB_URL + '/json'
...
class Deluge(commands.Cog):
    ...
    def post(self, method, params, use_cookies):
        ...
        r = self.session.post(WEB_URL, headers=header, data=json.dumps(data),
                              cookies=(self.session.cookies if use_cookies == True else None))
        ...
        for k, v in data['result'].items():
            if v['ratio'] not in ['-1', '0']:
                if len(torrents) > 1800:
                    ...
                    torrents += "#{queue} | {ratio} | {name} | {id})\n".format(queue=v['queue'], ratio=v['ratio'],
                                                                               name=v['name'], id=v['hash'])
        await ctx.send(torrents + "```")
        ...
        for k, v in data['result'].items():
            if 0.0 < float(v['progress']) < 100.0:
                if len(torrents) > 1800:
                    ...
                    torrents += "#{queue}\t | {progress}%\t | {name}\n".format(queue=v['queue'],
                                                                               progress=("%.2f" % v['progress']),
                                                                               name=v['name'])
        await ctx.send(torrents + "```")
