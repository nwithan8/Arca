"""
Interact with the Metropolitan Atlanta Rapid Transit Authority's API, get live train schedules and route times
...
from MARTA.martapi.api import get_buses, get_trains
import re
import json
import requests
...
class MARTA(commands.Cog):
    ...
    async def marta_trains(self, ctx: commands.Context, station: str = None, direction: str = None, line: str = None):
        ...
        if not found:
            ...
            if trains:
                if direction is not None and direction.lower() in ['n', 's', 'e', 'w', 'north', 'south', 'east', 'west',
                    ...
                    if direction.lower().startswith('n'):
                        ...
                        direction = 'W'
                    if line is not None and line.lower() in ['g', 'r', 'b', 'gold', 'red', 'green', 'blue', 'y',
                                                             'yellow', 'o', 'orange']:
                        if line.lower().startswith('r'):
                            line = 'RED'
                        elif line.lower().startswith('y') or line.lower().startswith('o') or line.lower().startswith(
                                'go'):
                            line = 'GOLD'
                ...
                if final_trains:
                    ...
                    for t in final_trains:
                        final_message = final_message + t.line.capitalize() + " line train to " + t.destination + " (" + t.direction + "): " + (
                            "Arriving in " + t.waiting_time if t.waiting_time[0].isdigit() else t.waiting_time) + "\n"
    ...
    async def marta_time(self, ctx: commands.Context, startPoint: str, endPoint: str):
        ...
        if not valid_start:
            ...
            time = 0
            if (stations[startPoint][4] != stations[endPoint][
                4]):  # one is ns, other is ew (5points is nsew, but time = 0, so not effect)
                # s -> 5 + 5 -> e
            ...
            await ctx.send("It takes approximately " + str(time) + (
                " minutes" if time > 1 else " minute") + " to go from " + startPoint.title() + " to " + endPoint.title())
