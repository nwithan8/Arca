...
class PlexManager(commands.Cog):
    ...
    def add_user_to_db(self, discordId, plexUsername, note, serverNumber=None):
        ...
        if note == 't':
            ...
            query = "INSERT OR IGNORE INTO users (DiscordID, PlexUsername, ExpirationStamp" + (", ServerNum" if serverNumber != None else "") + ", Note) VALUES ('" + str(discordId) + "','" + str(plexUsername) + "','" + str(timestamp) + (("','" + str(serverNumber)) if serverNumber != None else "") + "','" + str(note) + "')"
            cur.execute(str(query))
            query = "UPDATE users SET ExpirationStamp = '" + str(timestamp) + "' WHERE PlexUsername = '" + str(plexUsername) + "'"
